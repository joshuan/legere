...
| `ImageTool` | `normalizeOrientation(source)`, `toJpegPreview(source, {maxDim})`, `dimensions(source)`, `correctPage(source)`, `contentBox(source)`, `applyCrop(source, crop)`, `grayscaleRaster(source, maxDim)` | `SharpImageTool` |
| `EmbeddingProvider` | `embed(texts[]): number[][]`, `isConfigured` | `OpenAiCompatEmbeddings` (fetch, `EMBEDDINGS_*` env); like the classifier and transcriber adapters it preserves a `429` as a typed interruption with the parsed, bounded `Retry-After` deadline, which the shared service gate turns into the instance-wide FIFO hold of `05 §5.4b` |
