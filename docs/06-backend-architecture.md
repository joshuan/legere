...
| `DocumentAnalyst` | `analyze(excerpt, document types[], subject kinds[], known subjects[], language, pages[]): { title, description, typeSlug, languages, country, city, people, date, subjects, textQuality, usage }`, `extractFields(schema, excerpt, pages[]): { values, usage }` — the fields step (`05 §5.5` step 5), same provider, same gate, validated in code per field, `isConfigured`, `endpoint` | `OpenAiCompatAnalyst` (chat-completions JSON answer; the pages travel as images) |
