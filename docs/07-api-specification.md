...
| `GET /api/admin/people/merge-suggestions` | 🔒ᴬ | → `{ state, groups: [{ ids[≥2], name, aka[] }] }` — the analyst's reading of the living catalogue (05 §5.6c): rows it takes for one person, the spelling it would keep, the distinct other spellings. Computed on request and cached in-process against the catalogue's content; **nothing stored, a refusal never remembered**. At most 20 groups. `state` is one of three, because two of them are not the same silence: `ANSWERED` — the analyst was asked and these are its groups, empty ones included; `UNCONFIGURED` — there is no analyst on this instance; `UNAVAILABLE` — it was asked and could not answer, which is **not** an answer of none. Never an error in any of the three; `groups` is empty unless `ANSWERED` |
...
| `GET /api/admin/subject-kinds/merge-suggestions` | 🔒ᴬ | → `{ state, groups: [{ ids[≥2], name, aka[] }] }` — the analyst's reading of the kinds catalogue (05 §5.6c), on the people endpoint's terms, three-valued `state` included |
