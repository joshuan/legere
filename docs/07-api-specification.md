...
| 422 | `FILE_NOT_PDF` | ordering or turning the pages of something that has none — only a PDF does (`03 §3.3.16`) |
| 422 | `DOCUMENT_LAST_PAGE` | an edit that would leave a document with no pages at all: removing its last page, or moving every page of it away (`05 §5.6`). The page counterpart of `DOCUMENT_LAST_FILE`, and 422 rather than 409 because what is wrong is the request's own arithmetic against a list the caller was just shown, not a state it collided with. A cut past the last page is `VALIDATION_FAILED` instead — that names a page the document does not have, which is a position outside the list like any other |
| 422 | `DOCUMENT_WOULD_HAVE_NO_READERS` | a composition that would leave a document holding no library page when it had one and has no creator — a document a scan made, whose readers are exactly the people its libraries reach (`03 §3.4a`, `05 §5.6`). Replacing its only library file, splitting the last one off it, removing the last page that reads one, moving them all away, or cutting a part out of it that keeps none. 422 for the reason `DOCUMENT_LAST_PAGE` is: the request's own arithmetic against a list the caller was just shown. It is refused for **every** document the operation touches, and refused *before* anything is written |
| 422 | `DOCUMENT_TOO_MANY_FILES` | an add, a restore, a split or a combine that would leave a document reading more than 200 distinct **files** (`05 §5.4a`). Every one of them is opened whole, converted and held at once by the canonical build, so the count is what decides that step's peak memory — and it is counted in files rather than pages, because a four-hundred-page scan is one file opened once. 422 for the reason `DOCUMENT_LAST_PAGE` is: the request is well formed and the document is in no conflicting state; it is the size of what was asked for |
| 422 | `LINK_SELF` | linking a document to itself |
| 422 | `CATALOGUE_FULL` | a create that would push a catalogue past its instance ceiling — 10 000 living people, 20 000 living subjects, 500 living subject kinds (`08 §8.4`, SEC-51, SEC-56). Counted over living rows, so merges and deletes make room again; the numbers are far past any honest archive and exist so that one account cannot fill a namespace every other user reads. 422 for the reason `DOCUMENT_TOO_MANY_FILES` is: the request is well formed and collides with nothing; it is the size of what it would leave behind |
...
| `POST /api/auth/register/verify` | — | `{ email, code, inviteToken?, resetToken? }` → `{ ticket, expiresAt }`. 🔒 The token is the one step 1 accepted, echoed back: an attempt is charged only to a caller who proves they hold the link the series was made from, and a caller who does not is answered `400 EMAIL_CODE_INVALID` before the counter moves ([`08 §8.1.3`](./08-auth-and-authorization.md#813-the-three-account-setup-steps-shared-by-onboarding-invites-and-password-resets) step 2, SEC-57). Only the tokenless onboarding series has no link to prove |
...
| `GET /api/libraries/:id/browse?path=&cursor=` | 🔒 | virtual folder view derived from FileRef paths: `{ path, folders: [{ name, documentCount }], documents: { items: [DocumentListDto], nextCursor } }`; folders sorted by name, documents by title. 🔒 The **access rule applies here as everywhere** (`03 §3.4`): a `FileRef` in this library is not a proof that the document is readable — the file it points at may be a `MANAGED` one an upload created and a scan later deduplicated onto (`05 §5.3`) — so the list is filtered by the same predicate `GET /api/documents/:id` answers with, and a browse never shows what a detail refuses. 🔒 `path` names a folder and nothing else: `%` and `_` are letters here, matched literally rather than as the `LIKE` wildcards they are underneath (`05 §5.1`) |
...
| `DELETE /api/documents/:id` | 🔒ᴬ | **hard delete** — the one endpoint that destroys user data (03 §3.3.10, ADR-015 as amended). The document row and everything cascading from it (journal, chunks, people/subject links, `document_pages`), and its collection items are deleted, and so are its artifacts (09 §9.2). **The files it leaves with no live page go to the trash** (03 §3.3.10, 05 §5.7a) rather than to the incinerator — their bytes are the one part of a document no rebuild can recover — and a file still read by a page of another document stays exactly where it is. Every `FileRef` of those files is kept and set `EXCLUDED`, so the bytes stay on the read-only volume and no scan ingests them again (03 §3.3.9). Not reversible; `{ ok: true }`. The rows go first and the objects after, so a bucket that refuses mid-way leaves orphans the hourly `maintenance` sweep collects (09 §9.2) rather than rows pointing at nothing |
...
| `GET /api/documents/:id/events` | 🔒 | paginated, newest first → `{ items: DocumentEventDto[], nextCursor }` — the document's history (03 §3.3.18). Same access as the document itself, and the entries are redacted for a non-admin exactly as §3.3.18 says: no `endpoint`, no `path` on a `LIBRARY` entry, no `error`, and no `otherDocumentId`/`otherTitle` where the document at the other end is not one this reader may read |
...
| `sort` | Means | Ordering |
|---|---|---|
| `documentDate` | the date written on the paper (`03 §3.3.10`) | `document_date DESC NULLS FIRST, id DESC` — newest first, and **the undated before everything**: inside this order a document whose date nobody has read yet is the one still wanting attention, and NULLS LAST would file it behind a century of dated ones |
| `createdAt` *(default)* | when Legere first saw it | `created_at DESC, id DESC` — the order every list had before this existed, and the one a list has again when nobody says otherwise: the question an archive is opened with is *what arrived since I was last here*, and the date on the paper cannot answer it — a receipt from 2019 scanned this morning is the newest thing in the archive and the oldest thing on the shelf |
...
`DocumentPageDto` = `{ id, position, fileId, pageIndex: number | null, turn: { quarterTurns: 0 | 1 | 2 | 3, mirrored: boolean } | null, crop: { points: [[x,y] ×4] } | null, cropSource: 'NONE' | 'AUTO' | 'MANUAL' }` — **the document itself, in order** (`03 §3.3.17`, ADR-025): one entry per page, each naming the file it is read from and which page of it. `position` is 0-based and contiguous, and it is the position the composition endpoints below count in — so a caller inserts, cuts and moves against exactly the list it was last answered with, and never against a guess. `pageIndex` is `null` for the entry standing for a file **whole** while nobody has counted its pages, which is the one place a position covers more than one sheet of paper (`03 §3.3.17`). `files` beside it is the same list read the other way round, one row per distinct file, and everything a file row says about this document is derived from these entries.

`DocumentFileDto` = `{ id, position, name, mimeType, ext, sizeBytes, origin: 'LIBRARY' | 'MANAGED', available: boolean, crop: { points: [[x,y] ×4] } | null, cropSource: 'NONE' | 'AUTO' | 'MANUAL', rotation: { quarterTurns: 0 | 1 | 2 | 3, mirrored: boolean } | null, pageOrder: number[] | null, pageRotations: number[] | null, pageCount: number | null, isImage, refs: [{ libraryId, libraryName, path, status }], storageKey: string | null }` — one row per **distinct file** the document's pages are read from, ordered by where that file's first page sits (`03 §3.3.17`). 🔒 **Everything a file says about this document is read off those pages**, not off the file: `crop`, `cropSource` and `rotation` are its page's, an image being one page; `pageOrder` is the order this document holds its pages in, `null` where they stand as they arrived; `pageRotations` is one quarter turn per page of the file, by the file's own index, `null` where nothing is turned — which is what the **Turned** tag of `11 §11.5a` is read from. A file whose pages nobody has counted is one entry standing for it whole (ADR-025), and answers `null` to all of them. `pageCount` is the file's own, the count the last canonical build wrote down (`03 §3.3.16`), `null` until one has — which together with `pageOrder` is what says whether a row has pages to arrange at all (`11 §11.5a`); `refs` lists only libraries visible to the caller (ADMIN sees all) and is empty for a managed file. **A location is answered for every file**, and the two fields divide it: `refs` for bytes on a volume, `storageKey` for the object in the bucket a `MANAGED` file's bytes lie under (`09 §9.2`), null for a `LIBRARY` file, which has no object at all. 🔒 The key is a location and not a way in — the bucket is private and only a signed URL reads it, issued by an endpoint that has already passed the access check — and it discloses nothing new either: the layout is `files/{fileId}/original.{ext}` and both halves are already on this DTO.
...
| `GET /api/admin/people/merge-suggestions` | 🔒ᴬ | → `{ state, computedAt, groups: [{ ids[≥2], name, aka[] }] }` — the analyst's reading of the living catalogue (05 §5.6c): rows it takes for one person, the spelling it would keep, the distinct other spellings. Computed on request and cached in-process against the catalogue's content; `computedAt` is the moment this reading was computed, so the screen can say when it last looked, and `?refresh=1` drops the cached reading and asks anew — the recompute of 11 §11.12a. **Nothing stored, a refusal never remembered**. At most 20 groups. `state` is one of three, because two of them are not the same silence: `ANSWERED` — the analyst was asked and these are its groups, empty ones included; `UNCONFIGURED` — there is no analyst on this instance; `UNAVAILABLE` — it was asked and could not answer, which is **not** an answer of none. Never an error in any of the three; `groups` is empty unless `ANSWERED` |
| `POST /api/admin/people/merge-preview` | 🔒ᴬ | `{ ids[≥2] }` → `{ available, name?, aka?, note? }` — the same reading for rows an admin selected by hand, so the merge dialog opens tidy (11 §11.12a). `note` is the note the merge should keep, composed by the analyst from everything the merged rows carried (05 §5.6c) and bounded by the note's own contract limit. `404 PERSON_NOT_FOUND` for an id that is not a living person; `available: false` when the analyst is unconfigured or its answer did not parse — the dialog then falls back to the raw prefill |
...
| `GET /api/admin/subjects/merge-suggestions` | 🔒ᴬ | → `{ state, computedAt, groups: [{ ids[≥2], name, kindId, aka[] }], placeholders: [id] }` — the analyst's reading of the things catalogue (05 §5.6c), kind-aware: a group may fold rows across duplicate kinds, and `kindId` is the kind the survivor keeps, always one the merged rows already have. `placeholders` are rows whose name is a kind rather than a thing, offered for deletion. Same terms as the people endpoint: computed on request, cached in-process with `computedAt` and `?refresh=1`, nothing stored, the same three-valued `state`, and both lists empty unless `ANSWERED` |
| `POST /api/admin/subjects/merge-preview` | 🔒ᴬ | `{ ids[≥2] }` → `{ available, name?, kindId?, aka?, note? }` — the tidy reading for a hand-picked selection, the kind and the composed note included (11 §11.12a, 05 §5.6c). `404 SUBJECT_NOT_FOUND` for an id that is not a living thing; `available: false` degrades to the raw prefill |
...
| `GET /api/admin/subject-kinds/merge-suggestions` | 🔒ᴬ | → `{ state, computedAt, groups: [{ ids[≥2], name, aka[] }] }` — the analyst's reading of the kinds catalogue (05 §5.6c), on the people endpoint's terms: three-valued `state`, `computedAt`, `?refresh=1` |
| `POST /api/admin/subject-kinds/merge-preview` | 🔒ᴬ | `{ ids[≥2] }` → `{ available, name?, aka?, note? }`, the composed note included (05 §5.6c); `404 SUBJECT_KIND_NOT_FOUND` for an id that is not a living kind |
...
| `DELETE /api/admin/document-types/:id` | 🔒ᴬ | soft delete; resets documents to NONE → `{ ok, documentsReset }`. 🔒 The reset runs in bounded batches **outside** one transaction, and the soft delete follows it: the size of this cascade is not the admin's to choose — it is however many documents ordinary users filed under the type — and each of those rows carries a `type_id` index, so its update writes a new tuple into every index the row has, the GIN over the whole of its Markdown included (`04 §4.3`). As one statement inside one interactive transaction it stopped finishing past a few thousand documents and answered an untyped `500`, which made a documented admin action impossible on exactly the archives that need it. An interrupted call leaves the type alive over a partly-cleared set and repeating it finishes the job |

### Collections & sharing
| Method & path | Auth | Notes |
|---------------|------|-------|
| `GET /api/collections` | 🔒 | own + shared-with-me (flag `sharedByMe`/`sharedWithMe`), sorted by name. 🔒 `itemCount` is what **this** caller may list, not what the collection holds (`03 §3.3.14`): a share carries its owner's own work and nothing else, so a number counting the rest would say how many documents the grantee was refused |
| `POST /api/collections` | 🔒 | `{ name, description? }` |
| `GET /api/collections/:id` | 🔒 | canReadCollection; items = documents the caller can read (03 §3.3.14), paginated, and `itemCount` counts that same set — the sharpest form of the leak would be a response stating 40 beside a list of 0 |
...
Every one of them changes the composition and therefore enqueues a canonical rebuild + the rest of
the pipeline; all of them answer with the whole `DocumentDetailDto`, because a composition change is
never local — which is also how a caller gets the page list its next request will index into.

🔒 **Two rights, not one** (`03 §3.4a`, `08 §8.5`). The **arrange** column below is `canEditDocumentMeta`
— the rule that lets any reader of a library document correct its title, applied to where its pages
stand. The **destroy** column is the creator's or an `ADMIN`'s, and a document a scan made has no
creator, so on a library document it is an `ADMIN`'s: those are the three operations that make a page
stop being read at all. Every one of them may also answer
`422 DOCUMENT_WOULD_HAVE_NO_READERS` (§7.1).

🔒 **And every one of them may answer `409 DOCUMENT_CHANGED`** (§7.2). These routes rewrite the
document's whole page list from the reading they were given — the list the caller was last answered
with — so a rewrite computed against a list that has since moved is refused rather than written
(`03 §3.3.17`). Nothing is changed by the refusal, and what to do about it is to read the document
again and repeat the edit against the list that comes back. The two writes that cannot lose anything
are outside it: an upload with no `?at=` appends, and so do the pages a combine moves into its
target.

| Method & path | Auth | Notes |
|---------------|------|-------|
| `POST /api/documents/:id/files?at=` | 🔒 arrange | the file **is** the body (as `POST /api/documents`, and exempt from the body parsers on the same terms — `05 §5.1a`), name in `X-Legere-Filename` (`X-File-Name` is accepted too). Its pages go **at `?at=`**, a 0-based position in this document's `pages` (`03 §3.3.17`), and after the last page when it is absent — which is what puts a photograph between page two and page three of a five-page PDF (`05 §5.6`). `422 VALIDATION_FAILED` for a position past the end of the list; `413` over `UPLOAD_MAX_BYTES`; `409 FILE_ALREADY_IN_DOCUMENT` when those bytes already live in another document |
| `PATCH /api/documents/:id/pages` | 🔒 arrange | `{ order: [pageId, ...] }` — the complete order, **every page of this document exactly once** (`422 VALIDATION_FAILED` otherwise, naming nothing partial): one request and one truth, the way the page strip already sends a whole permutation (`11 §11.5a`). This is how a page is moved to a position, and how the pages of one file are put back into the order they arrived in without disturbing anything else |
| `PATCH /api/documents/:id/pages/:pageId` | 🔒 arrange | `{ crop?: { points: [[x,y] ×4] } \| null, turn?: { quarterTurns: 0 \| 1 \| 2 \| 3, mirrored: boolean } \| null }` — how **one page** lies and how much of it is paper (`03 §3.3.17`). At least one of the two keys, or `422 VALIDATION_FAILED`: a PATCH that changes nothing is not an edit. Both together are one edit and therefore one rebuild, which is what the crop editor sends (`11 §11.5c`). **`crop`**: normalized `0…1`, clockwise from top-left; `null` clears it and the page goes back into the canonical whole. It is accepted on **any** page, an image's or a PDF's, because the build honours a crop on a page of a PDF by rendering the page and warping it exactly as it warps a photograph (`05 §5.5` step 1) — a scanned page is already raster and loses nothing by it, and a vector page cropped becomes raster, which is what somebody who dragged its corners asked for. Setting one makes `cropSource` `MANUAL`, which no rebuild overwrites; clearing it returns it to `NONE`, so the machine may answer again. **`turn`**: the mirror first, then that many quarter turns clockwise; `null` restores the way the page arrived. 🔒 `mirrored: true` is **a page of an image's own** — `422 FILE_NOT_IMAGE` on any other page, which is the one thing that code now means among the edit routes. `404 PAGE_NOT_FOUND` for a page this document has not got |
| `DELETE /api/documents/:id/pages/:pageId` | 🔒 **destroy** | the page leaves the document and the rest close up behind it. `404 PAGE_NOT_FOUND` for a page of another document, `422 DOCUMENT_LAST_PAGE` for the only one it has, `422 DOCUMENT_WOULD_HAVE_NO_READERS` for the last page of a library document that reads a library file. 🔒 The file it was read from goes to the **trash** with `PAGE_REMOVED` only if no live page anywhere still reads it (`05 §5.7a`) — which, with its refs `EXCLUDED`, is why this route **destroys** rather than arranges (`03 §3.4a`): a reader of a library document moves a page out instead |
| `POST /api/documents/:id/pages/move` | 🔒 arrange **on both** | `{ pageIds: [uuid, ...] (1…2000), documentId: uuid \| null, at?: number }` → `{ document, movedToDocumentId }`. The named pages leave this document and join `documentId` at `at` — 0-based in *that* document's pages, appended when absent — or a **new** document made to hold them when `documentId` is `null`, which takes this document's owner and nothing else (`05 §5.6`); `at` may not be sent with `null`, there being one place to put them. 🔒 Refused **whole**: `404 DOCUMENT_NOT_FOUND` for a target the caller cannot read, `403 FORBIDDEN` for one they may not edit, `422 VALIDATION_FAILED` for a page that is not this document's, for the same page twice, for the document itself as target, or for a position past the end of the target's list, `422 DOCUMENT_LAST_PAGE` for a move that would empty this document, and `422 DOCUMENT_WOULD_HAVE_NO_READERS` for one that would strip either end of its last library page while it has no creator. Both documents rebuild |
| `POST /api/documents/:id/split` | 🔒 arrange | `{ at: [position, ...] (1…20) }` → `{ document, splitDocumentIds }` — cuts the document at those 0-based page boundaries into two or more, the entries dividing between them in order: the first part stays here, each later one becomes a document that takes this one's owner and access, is titled after the file its first page comes from and inherits nothing else (`05 §5.6`). 🔒 **No bytes are copied and no file is extracted** — the same file is read by pages in two places (ADR-025) — and the parts are **linked** to each other and to the original (`03 §3.3.23`). `422 VALIDATION_FAILED` for a boundary at or before the first page, past the last, or named twice; `422 DOCUMENT_WOULD_HAVE_NO_READERS` when a part would keep no library page and take no creator with it. Every part rebuilds |
| `PATCH /api/documents/:id/files` | 🔒 arrange | `{ order: [fileId, ...] }` — the complete order, every file exactly once (`422 VALIDATION_FAILED` otherwise); the pages of each file move as a block and keep the order this document reads them in. The coarse half of the route above it, kept for a screen that shows files rather than pages |
| `PATCH /api/documents/:id/files/:fileId` | 🔒 arrange | `{ pageOrder?: number[] \| null, pageRotations?: number[] \| null }` — what one file says about **its own pages as a set**, by the file's own 0-based indices, which is a thing to say about a file and not about any one page. That is all this route is since ADR-025: a crop and a turn belong to the page that carries them and are asked of the route that names it, `PATCH …/pages/:pageId` above — a crop, because since ADR-025 every page takes one and this route could only ever offer it to an image; a turn, because an image is one page and two ways of writing one row is how they drift apart. At least one of the two keys, or `422 VALIDATION_FAILED`: a PATCH that changes nothing is not an edit. **`pageOrder`**: the complete order of that file's pages, sent whole as a file reorder is (`03 §3.3.17`) — every 0-based index of the file exactly once, at most 2000 of them, checked against the page count the last canonical build recorded; `422 FILE_NOT_PDF` for anything but a PDF, and `422 VALIDATION_FAILED` for a list that is not a permutation of exactly that file's pages or for a file whose pages no build has counted yet. `null` restores the order the file arrived in. **`pageRotations`**: one quarter turn per page, `0…3` clockwise, in the file's own page order — exactly as many entries as that same recorded page count, refused on the same two terms as `pageOrder` and with the same two codes; `null` restores the way they arrived. Saving either enqueues the rebuild every composition change enqueues (`05 §5.6`), once |
| `POST /api/documents/:id/files/:fileId/replacement` | 🔒 **destroy** | **replace**: the body is the new file, on the same terms as `POST /api/documents/:id/files` above, and it stands where the named file's **first page** stood, the old file's other entries going with it → the whole `DocumentDetailDto`. It arrives as one entry standing for the new file whole, with no turn and no crop — different bytes are a different paper (`05 §5.6`). 🔒 The replacement reaches **every** document holding a live page of the old file (ADR-025): all of them are rewritten the same way, all of them rebuild, and the caller must be allowed to destroy content in every one of them or the request is refused whole with `409 FILE_READ_ELSEWHERE`. The file it replaces goes to the trash with `REPLACED` (`05 §5.6`, `05 §5.7a`), its refs `EXCLUDED`, and stays listed under its successor as an earlier version. `409 FILE_ALREADY_IN_DOCUMENT` when the new bytes are a live file of another document; bytes that are an earlier version of this same file are taken back out of the trash rather than refused. 🔒 It **destroys** — the bytes a page reads are substituted — so it is the creator's or an `ADMIN`'s (`03 §3.4a`); `422 DOCUMENT_WOULD_HAVE_NO_READERS` when the file it replaces is the last library file of any document it reaches; `409 DOCUMENT_CHANGED` when a document's page list moved while the request was being read |
| `DELETE /api/documents/:id/files/:fileId` | 🔒 arrange | **split**: the file leaves — with the pages this document held of it, their turns and their crops — and becomes its own document, which takes **this document's owner** and nothing else (`05 §5.6`) → `{ document, splitDocumentId }`. `409 DOCUMENT_LAST_FILE` when it is the only one, `422 DOCUMENT_WOULD_HAVE_NO_READERS` when it is the last **library** file of a document that has no creator, or when the file that leaves is a purely uploaded one of a document that has none — the part that leaves takes no owner either, so nobody could read it |
...
| `GET /api/documents/:id/files/:fileId/pages/:page/thumb` | 🔒 | 302 → signed URL of a small JPG of one page of the **original** file — the pages as they arrived, which is what somebody putting them in order looks at (`11 §11.5a`). `:page` is 0-based, the way `pageOrder` counts (`03 §3.3.16`). Rendered on the first request and kept in the bucket under `files/{fileId}/pages/{n}.jpg` (`09 §9.2`); every request after that is a redirect to the object, and it may be cached for as long as anybody likes, because file bytes are immutable. `422 FILE_NOT_PDF` for a file that has no pages, `404 NOT_FOUND` for a page beyond the count the last canonical build recorded — including every page of a file no build has opened yet, since nothing yet says how many there are. 🔒 **A stored turn does not reach it**: the picture is the page as it arrived, and the strip turns what it draws (`11 §11.5a`). The cache key is bytes that cannot change, which is the whole reason this may be kept for ever — keying it on a turn instead would purge and re-render an artifact on every press of a rotate button. 🔒 Behind the same access guard as the file's own content: it is the same bytes, one page at a time |
| `POST /api/documents/:id/combine` | 🔒 arrange here, **destroy** each source | `{ documentIds: [uuid, ...] }` (1…50) — their files are appended in that order and the emptied documents are soft-deleted. 🔒 The caller must be able to **destroy** each source (`03 §3.4a`): absorbing a document ends it, which is what `DELETE /api/documents/:id` spends `@Roles('ADMIN')` on, and on a library document the answer is the same. Arranging the target is enough for the target. Each absorbed document's canonical PDF, preview and thumbnail are deleted once the transaction commits — nothing reaches a soft-deleted document again, so nothing else ever would (`09 §9.2`) |
...
🔒 **The route shares the `search` budget of `08 §8.4`** — 30 calls per 60 s counted against the
token's owner rather than the machine the assistant runs on, because `search_documents` is the same
search as §7.3 and spends the same outbound embeddings call (SEC-74). A `429` here is an HTTP
failure, like authentication and unlike everything else on this route: it is refused before the
protocol is read.
