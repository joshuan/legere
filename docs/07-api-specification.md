...
| 422 | `FILE_NOT_IMAGE` | cropping something that is not an image, or turning it as one — an image takes one turn and a mirror (`03 §3.3.16`) |
...
| `PATCH /api/documents/:id/files/:fileId` | 🔒 canEdit | `{ crop?: { points: [[x,y] ×4] } \| null, rotation?: { quarterTurns: 0 \| 1 \| 2 \| 3, mirrored: boolean } \| null, pageOrder?: number[] \| null, pageRotations?: number[] \| null }` — what one file says about itself. At least one of the four keys, or `422 VALIDATION_FAILED`: a PATCH that changes nothing is not an edit. In practice a body names the pair its file has, because an image and a PDF share none of them — only an image is cropped and turned as one picture, only a PDF has pages to order and to turn one at a time. **`crop`**: normalized `0…1`, clockwise from top-left; `null` clears it; `422 FILE_NOT_IMAGE` for anything but an image. **`rotation`**: which way up the image lies (`03 §3.3.16`) — the mirror first, then that many quarter turns clockwise; `null` restores the way it arrived; `422 FILE_NOT_IMAGE` for anything but an image. **`pageOrder`**: the complete order of that file's pages, sent whole as a file reorder is (`03 §3.3.16`) — every 0-based index of the file exactly once, at most 2000 of them, checked against the page count the last canonical build recorded; `422 FILE_NOT_PDF` for anything but a PDF, and `422 VALIDATION_FAILED` for a list that is not a permutation of exactly that file's pages or for a file whose pages no build has counted yet. `null` restores the order the file arrived in. **`pageRotations`**: one quarter turn per page, `0…3` clockwise, in the file's own page order — exactly as many entries as that same recorded page count, refused on the same two terms as `pageOrder` and with the same two codes; `null` restores the way they arrived. Saving any of them enqueues the rebuild every composition change enqueues (`05 §5.6`), once |
