...
| a file's pages go in at a position, and a photograph lands between page two and page three | `test/e2e/document-pages.e2e.test.ts` — puts a photograph between page two and page three of a five-page scan; refuses a position past the end |
...
| each half of a split builds a canonical out of its own pages | `src/server/application/documents/build-canonical.test.ts` — builds each half of a split out of its own pages of the one file |
