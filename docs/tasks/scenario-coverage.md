...
| CSRF fail-closed | `test/e2e/login.e2e.test.ts` — CSRF (fail-closed origin check), six tests |
...
| Scenario | Test |
|---|---|
| pdf-with-text, no OCR | `src/server/application/jobs/handle-document-process.test.ts` — leaves a PDF that carries its own text alone |
| scanned pdf → OCR | `src/server/application/jobs/handle-document-process.test.ts` — OCRs a merged PDF whose text layer is too thin, and stores the searchable one; measures the text layer per page, not in total |
| office → canonical | `src/server/application/jobs/handle-document-process.test.ts` — converts an office document, and plain text through the same door |
| image → a page of the canonical PDF (there is no trim step: `02` ADR-025 lays an image on a page, and the preview reads the canonical like everything else's) | `src/server/application/jobs/handle-document-process.test.ts` — lays an image on a page, and applies the crop it carries first; previews an image document the same way as any other, because it is a PDF by now |
| txt/md → converted into the canonical (no passthrough: `05 §5.5` step 1 sends plain text and Markdown through Stirling like an office file) | `src/server/application/jobs/handle-document-process.test.ts` — converts an office document, and plain text through the same door |
| unsupported → SKIPPED | `src/server/application/jobs/handle-document-process.test.ts` — leaves out a file nothing can render, and says the step is incomplete; builds nothing for a document nothing can render, and fails no step over it; `test/integration/document-process.integration.test.ts` — leaves a document nothing can render settled, without any artifact |
| step failure isolates | `src/server/application/jobs/handle-document-process.test.ts` — keeps a markdown failure from touching the canonical or the preview; records a failure with the message, not just the status |
...
| USER vs RESTRICTED library: files | `test/e2e/document-files.e2e.test.ts` — refuses every file route exactly like the metadata routes; lets a granted user download and compose the same document |
...
| no configured analyst → `UNCONFIGURED`, never an error | `test/e2e/people-merge-suggestions.e2e.test.ts` — answers UNCONFIGURED with no analyst, never an error; `test/e2e/catalogue-suggestions.e2e.test.ts` — answers UNCONFIGURED on subjects and kinds alike, never an error |
| an analyst that could not be asked answers `UNAVAILABLE`, never an empty reading | `src/server/application/people/suggest-people-merges.test.ts` — reports an outage as UNAVAILABLE, never as an answer of none; `src/server/application/subjects/suggest-subject-merges.test.ts` — answers each of the three states, an outage never as an answer of none; `test/e2e/catalogue-suggestions-unavailable.e2e.test.ts` — answers UNAVAILABLE on all three endpoints, never an error and never an empty answer |
| a failure is not remembered: the next request asks again | `src/server/application/subject-kinds/suggest-subject-kind-merges.test.ts` — answers UNAVAILABLE when the analyst could not be asked, and asks again next time; `test/e2e/catalogue-suggestions-unavailable.e2e.test.ts` — asks again on the next request, because a refusal is never remembered |
| a failure is logged with what an operator can act on, and never the rows | `src/server/infrastructure/ai/openai-compat-catalogue-analyst.test.ts` — says a failure out loud, naming the reading and never the rows |
| the screens say the analyst could not be asked instead of showing nothing | `src/web/screens/people/people-screen.test.tsx` — says the analyst could not be asked, instead of showing nothing; `src/web/screens/subjects/subjects-screen.test.tsx` and `src/web/screens/subject-kinds/subject-kinds-screen.test.tsx` — the same |
| a catalogue past the cap is asked in bounded chunks, and the answers unioned | `src/server/infrastructure/ai/openai-compat-catalogue-analyst.test.ts` — cuts a catalogue past the cap into bounded calls and unions their answers; asks once, with the rows untouched, for a catalogue that fits in one call |
| one name in two scripts is asked about in one call | `src/server/application/catalogues/catalogue-chunks.test.ts` — reads one name out of two scripts into one key; puts the same name in two scripts in the same chunk; `src/server/infrastructure/ai/openai-compat-catalogue-analyst.test.ts` — keeps a name in two scripts inside one call |
...
| receipt vendor/date/country/currency/amount filters and purchase/upload/amount orders operate over the whole paginated archive | `test/e2e/receipts.e2e.test.ts` — filters extracted facts and paginates each named receipt order; `src/shared/contracts/receipts.test.ts` — receipt list query; `src/server/infrastructure/persistence/cursor.test.ts` — receipt cursor |
| receipt processing counts cover the live archive independently of retained jobs; bounded retries preserve data, exclude live work and commit atomically | `test/e2e/receipts.e2e.test.ts` — admin receipt processing: whole-archive counts, bounded batches and actor events, concurrent retries, rollback on enqueue failure, paused/unconfigured extraction, batch validation and admin/same-origin authorization |
| receipt retry controls confirm the chosen batch, refresh counts and explain why retry is unavailable | `src/web/screens/admin-queue/admin-queue-screen.test.tsx` — shows archive counts separately from queue jobs with translated receipt controls; confirms the chosen retry batch, posts once and refreshes archive counts; disables receipt retry when paused, unconfigured, unregistered or empty |
...
| the receipt UI has linkable filters/order, a full-width useful table, separate responsive detail, and keeps noisy caller text collapsed | `src/web/screens/receipts/receipts-screen.test.tsx` — uses the full receipt table with useful extracted facts and a responsive card list; reads filters and order from the URL, sends them to the API and keeps them linkable; refreshes a fresh upload into the list only under newest-upload-first order; `src/web/screens/receipt-viewer/receipt-viewer-screen.test.tsx` — shows the image, structured fields and item rows, with source text behind a disclosure; `src/web/widgets/app-shell/app-shell.test.tsx` — offers the top-level Receipts destination |
...
| Scenario | Test |
|---|---|
| DOC and DOCX are detected, uploaded and ingested from libraries, including DOCX metadata after the first 4100 bytes | `src/server/infrastructure/library/file-type-mime-detector.test.ts` — real Word fixtures, late ZIP metadata, bounded stream cleanup and renamed non-Word content; `test/e2e/uploads.e2e.test.ts` — Word uploads; `test/integration/scan-ingest.integration.test.ts` — Word library ingest |
| Word becomes a canonical PDF with a readable text layer and preview; removing a page leaves the original unchanged | `test/integration/stirling-pdf-toolbox.integration.test.ts` — real DOC/DOCX conversion and JPEG preview; `test/integration/canonical-build.integration.test.ts` — DOCX canonical, page count, removal and retained original |
| native DOCX Markdown preserves structure only for the whole source, with PDF fallback and normal service interruption handling | `src/server/application/jobs/handle-document-process.test.ts` — native Word Markdown: managed/library originals, removed/reordered/duplicated pages, merged files, empty/rejected/missing source and unavailable Docling; `src/server/infrastructure/pdf/docling-parser.test.ts` — one native DOCX submission without PDF windows or OCR |
...
| adding, reordering and splitting each rebuild the document | `test/e2e/document-files.e2e.test.ts` — appends the upload, stores its bytes and rebuilds the document; rewrites the order the client sends and rebuilds; gives the file a document of its own and leaves the rest behind (which asserts the rebuild of both halves) |
...
| splitting off a file yields a document of its own | `test/e2e/document-files.e2e.test.ts` — gives the file a document of its own and leaves the rest behind |
| the last file cannot be taken away (`DOCUMENT_LAST_FILE`) | `test/e2e/document-files.e2e.test.ts` — refuses to take the only file away |
| combining moves pages in the chosen order and soft-deletes the emptied documents | `test/e2e/document-files.e2e.test.ts` — moves the files over in the order given and buries the emptied documents |
| uploading bytes a document already holds is refused (`FILE_ALREADY_IN_DOCUMENT`) | `test/e2e/document-files.e2e.test.ts` — refuses bytes that already belong to another document |
...
| the migration turns every existing row into pages, over each of the three shapes | `test/integration/document-pages-migration.integration.test.ts` — turns a stored page order into that many entries, in that order, with the turns they named; turns a counted file into its pages, as they lie; holds a file nobody has counted as one entry standing for it whole, carrying its crop |
...
| a file's pages go in at a position, and a photograph lands between page two and page three | `test/e2e/document-pages.e2e.test.ts` — puts a photograph between page two and page three of a five-page scan; refuses a position past the end; `test/integration/canonical-build.integration.test.ts` — holds a photograph inserted between two pages of a PDF exactly where it was put |
| the whole order is sent at once, and a partial one is refused | `test/e2e/document-pages.e2e.test.ts` — rewrites the order the client sends, across the boundary between two files; refuses an order that is not the whole document |
| a page is removed, and a file with no live page anywhere goes to the trash | `test/e2e/document-pages.e2e.test.ts` — sends the file to the trash when its last page leaves; refuses the only page there is (`DOCUMENT_LAST_PAGE`) |
| a document is cut at a page into two, over the same file and with no bytes copied | `test/e2e/document-pages.e2e.test.ts` — cuts a twelve-page scan at eight, over the same file, and links the halves |
| each half of a split builds a canonical out of its own pages | `src/server/application/documents/build-canonical.test.ts` — builds each half of a split out of its own pages of the one file; `test/integration/canonical-build.integration.test.ts` — gives each half of a split its own pages of the one file and none of the other half’s |
...
| the crop is a quadrilateral applied as a perspective transform | `src/server/domain/entities/crop-geometry.test.ts` — `planCrop`: a rotated quad is a known homography: the corners and the diagonal crossing land on it; `warpPerspective`: a quarter turn transposes the image |
| corners are detected, and fall back to the content box | `src/server/domain/entities/page-detection.test.ts` — finds a page lying at an angle, and says which corner is which; answers nothing for a page that fills the frame, for noise, and the rest of the no-page frames (six "answers nothing for…" tests) |
| a MANUAL crop survives a rebuild | `src/server/application/documents/build-canonical.test.ts` — keeps a crop and a turn somebody chose across a rebuild that expands the entry, and across a second run. The line used to name `test/e2e/document-pages.e2e.test.ts`, which stores `cropSource: MANUAL` and never runs a rebuild at all: it proved the value was written, not that anything survived |
...
| Registration — 3 steps, email code (HMAC hash, TTL 10 min, ≤5 attempts, spendable only by the link's holder); the `User` exists only after step 3, via a single-use ticket | `test/e2e/registration.e2e.test.ts` — walks the three-step happy path and signs the new admin in; burns the record after five wrong codes; rejects an expired code and an expired ticket; rejects a ticket that was already used; lets the holder of an invite pass their code while a stranger burns guesses at it; still burns an invite series after five wrong codes from the holder; `src/server/application/auth/start-registration.test.ts` — sends a code valid for ten minutes; `src/server/application/auth/verify-email-code.test.ts` — counts wrong codes and burns the series on the fifth; issues a ticket valid for fifteen minutes on the right code; refuses to reuse a consumed series; proof that the caller holds the link (SEC-57): lets the holder of a reset link spend and pass its code; charges a stranger nothing, however many codes they try; is not satisfied by some other link of the same kind; still burns the series after five wrong codes from the holder; falls through to the series the caller can prove, not the one that sorts first; `src/server/infrastructure/auth/hmac-verification-codes.test.ts` — stores an HMAC under AUTH_SECRET rather than the code itself; is not reproducible by somebody who has the row but not the secret |
...
| Mutations — fail-closed `csrfOriginCheck` above the dispatcher; four named budgets counted per caller + per-email rate limiting; CAPTCHA on login/start | `test/e2e/login.e2e.test.ts` — CSRF (fail-closed origin check): rejects a mutation with no Origin or Referer; rejects a mutation from a foreign origin; accepts a same-origin mutation proven by Referer alone; leaves reads alone; guards every mutating method, not just POST; guards a mutation that never reaches /api at all; refuses further /api/auth requests from one address once the budget is spent; `test/e2e/account.e2e.test.ts` — refuses a replay past its budget without letting it reach the hasher, and login is unmoved; `test/e2e/search.e2e.test.ts` — the search budget: refuses one caller past their budget and leaves everybody else searching; is shared with the MCP route the assistant searches through; `test/e2e/catalogues.e2e.test.ts` — a namespace, not a scratchpad (SEC-56): refuses a burst of creates before the namespace fills; `src/server/infrastructure/throttling/sliding-window-throttler-storage.test.ts` — admits the budget and refuses the request past it; slides: a hit that has fallen out of the window is not counted; lets a blocked caller back in once the block has passed; leaves every other caller alone when one of them is blocked and comes back; counts two named budgets separately even under one key; forgets a key whose window and block have both passed; never holds more keys than its ceiling, however wide the burst; `src/server/infrastructure/auth/in-memory-login-attempts.test.ts` — sweeps streaks nothing can still be waiting on; caps the number of streaks it will hold at once; evicts the coldest streak rather than the one being written; `test/e2e/registration.e2e.test.ts` — enforces the per-email cap of one code per minute; enforces the per-email daily cap of five codes; `src/server/application/auth/login.test.ts` — rejects a failed CAPTCHA before checking the password; `src/server/application/auth/start-registration.test.ts` — rejects a failed CAPTCHA before touching any state; `src/server/infrastructure/auth/turnstile-captcha-verifier.test.ts` — fails closed when the verification endpoint errors or is unreachable; `src/web/features/login-form/login-form.test.tsx` — sends the token the widget minted, and will not submit before there is one; asks the widget for a fresh token after a failed attempt; renders nothing at all on a build with no site key; `src/web/features/auth-wizard/auth-wizard.test.tsx` — carries the widget's token into `register/start`, and mints a second one for the resend |
| API tokens: hashed at rest, shown once, mandatory expiry, revoked with the owner and with the admin-issued reset (§8.1.6); a mutating request carrying one is refused before routing (§8.2a) | `test/e2e/api-tokens.e2e.test.ts` — reads as its owner and shows the secret exactly once; is refused on every mutating method, before the route is even reached; carries the owner authority and no more; stops working when revoked, when expired, and when its owner is deactivated; does not survive the password reset an admin issued to take the account back; `src/server/application/auth/complete-registration.test.ts` — revokes the API tokens of a reset account along with its sessions; leaves the tokens of an unrelated account alone when an invite is accepted; `src/server/application/auth/authenticate-api-token.test.ts` — issues a prefixed token, stores only its hash, and answers with the owner; defaults the lifetime to the instance setting and honours a chosen one; `src/server/presentation/auth/session.guard.test.ts` — refuses a token on a mutating method even with the middleware gone (the second of the two layers §8.2a promises) |
| `passwordHash`/`tokenHash`/codes/tickets and email bodies are never serialized or logged | `test/e2e/request-logging.e2e.test.ts` — keeps invite and reset tokens out of request URLs and emitted logs; keeps neither token anywhere in everything the process has emitted; never says what somebody searched their archive for; never says what a document is called; `src/web/shared/lib/use-fragment-token.test.tsx` — reads a fragment token once and removes it from the current history entry; `src/server/infrastructure/logging/logger.options.test.ts` — drops the query string, so a search never says what was searched for; logs the shaped URL and nothing else of what the request carried; drops the credentials and the document names the old deny-list named; `src/server/infrastructure/email/log-email-sender.test.ts` — logs the recipient and the subject, and never the body; `test/e2e/security-events.e2e.test.ts` — keeps no password, code, ticket or token anywhere in anything it recorded; `test/e2e/users.e2e.test.ts` — returns the signed-in user and never their hash; `src/server/infrastructure/config/instance-view.test.ts` — keeps configured secrets out of the response entirely; names every key in the schema that looks like a credential |
