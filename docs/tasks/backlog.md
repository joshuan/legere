...
- [x] **M56.1 — One manager under three screens**
...
- [x] **M56.2 — The actions stand at the foot of the screen**
...
- [x] **M56.3 — The column that says when the paper last named it**
...
- [x] **M56.4 — The counts are doors on the kinds screen too**
...
- [x] **M56.5 — The analyst writes the note a merge keeps**
...
- [x] **M56.6 — Focus lands on the button you came for**
...
- [x] **M56.7 — The duplicates panel is always there, and says when it looked**
...
- [x] **M58.2 — An image too large to open is made smaller, not refused**
...
- [x] **M58.3 — A window with no time left says so, and does not spend an upload**
...
- [x] **M59.1 — A `Retry-After` response stops the service gate**
...
## M61 — One place to understand and control processing

pg-boss queues, the six durable states inside `document-process` and the five outbound-service gates
are sound mechanisms with different jobs. What is missing is one model explaining how they connect
and one application boundary which reads and changes them. This milestone adds that management
aggregate without turning queues, steps and services into one runtime abstraction or replacing the
sequential document handler with a per-step job DAG.

Tasks are strictly sequential by dependency. Compatibility routes remain during this milestone and
delegate to the same control plane; removing them is a later explicit decision, not an incidental
cleanup.

---

- [x] **M61.1 — The processing topology is stated once**
  **Goal:** one validated, read-only `ProcessingTopology` declaration describes every queue, the document pipeline, its dependency edges and every external-service edge.
  **Depends on:** M60 complete.
  **Docs:** [ADR-026](../02-architecture-overview.md#adr-026-one-processing-control-plane-three-execution-mechanisms), [`05 §5.4f`](../05-library-and-processing.md#54f-one-processing-control-plane), [`06 §6.3.5`](../06-backend-architecture.md#635-processing-management)
  **Acceptance:**
  - Shared contracts expose closed queue, step, service, dependency-condition and service-role enums; the topology contains `library-scan`, `file-ingest`, `document-process`, `maintenance`, the six §5.5 steps in execution order and the five §5.4b services.
  - `document-process` is the host of the document pipeline; dependency and service edges match the complete table in §5.4f, including Markdown's primary/fallback/optional/auxiliary resources, Stirling's page-image role in analysis/fields and the conditional analysis → fields dependency.
  - Queue kind/produces/configurability plus `policy`/`expireInSeconds` metadata is represented without framework objects or handler callbacks; the pg-boss adapter consumes or asserts those declarative values, and executable registries either derive identities from the topology or assert exact coverage against it.
  - No new generic queue/step/service base class, common status enum or graph executor is introduced; the current pg-boss adapter, durable document statuses and service gates keep their semantics.
  - The `as const` declaration is read-only by construction and Zod-cloned before publication rather than exposed as a mutable singleton. Focused tests parse the published shape, compare queue/step/service identities and order with the closed shared contracts, require dependencies to point backward, require each resource edge's service consumer list to contain the step, compare policy/expiry with the pg-boss adapter, and compare the executable worker bindings with all topology queues.

- [x] **M61.2 — One control plane owns load settings and their safe application**
  **Goal:** every processing load control is read and mutated through `ProcessingControlPlane`, with provenance, optimistic concurrency and compensation instead of a controller sequencing unrelated writes and restarts.
  **Depends on:** M61.1.
  **Docs:** [`03 §3.3.21`](../03-domain-model.md#3321-setting), [`05 §5.4f`](../05-library-and-processing.md#54f-one-processing-control-plane), [`06 §6.3.5`](../06-backend-architecture.md#635-processing-management), [`07 §7.3`](../07-api-specification.md#admin-processing), [`12 §12.4`](../12-build-config-run.md#124-envexample)
  **Acceptance:**
  - A single application service composes the existing queue/gate settings, reports every load-control leaf as effective value + default + `DEFAULT|OVERRIDE`, ignores invalid stored fields on read and preserves the existing `queue` row without a schema migration; analysis language stays the existing separate content preference and UI field.
  - Scoped commands exist for exactly one queue, the pipeline, one step or one service. Queue/service partials reject an empty setting set and every command rejects out-of-range values; nullable numbers clear only that override and pause booleans add/remove only that named pause, so no body can carry a second scope's draft.
  - Every command includes the snapshot revision it was based on; a stale revision is refused as a conflict with the current revision rather than overwriting a newer admin change. Changes to distinct scopes from concurrent callers remain serial and lossless after each caller refreshes.
  - Runtime changes use the `ProcessingWorkerRuntime` port and the §5.4f safe-apply protocol: validate/build under one mutation lock, persist the durable desired candidate, configure gates/only affected workers, and on failure persist the before-image with a new monotonic revision and re-apply it. The controller contains no worker/gate/settings ordering logic.
  - A queue command re-registers only that queue; a gate command updates existing waiters by their next acquisition; pipeline/step values which are read per job restart no worker. A resumed step performs bounded idempotent catch-up only after the setting is committed and reports the immediate enqueue count.
  - A failed desired-state persistence changes no runtime. Runtime failure is `PROCESSING_APPLY_FAILED`; successful recovery leaves the before-image durable under a newer revision, while failed recovery sets `DEGRADED`/`appliedRevision: null`. Startup reads the same durable effective settings and uses the same gate/worker adapter rules, registering all configured queues rather than calling the scoped live reconfigure operation. No partial case returns success; post-apply catch-up failure is `APPLIED_WITH_WARNINGS` and maintenance-safe.
  - `queue-settings.test.ts` covers default/override provenance, sparse numeric reset, legacy-row normalization, invalid stored fallback and monotonic restore. `processing-control-plane.test.ts` covers a serialized stale command, no-op, gate-only and affected-queue apply, desired-persist failure, restored runtime, failed compensation and bounded resume warnings; the scenario index under M61.8 is the authoritative map.

- [x] **M61.3 — One snapshot and one administrative API**
  **Goal:** the server returns topology, settings and current operational state as one coherent processing snapshot, and every new administrative command is exposed under one namespace.
  **Depends on:** M61.2.
  **Docs:** [`05 §5.4b–§5.4f`](../05-library-and-processing.md), [`06 §6.3.5`](../06-backend-architecture.md#635-processing-management), [`07 §7.3`](../07-api-specification.md#admin-processing)
  **Acceptance:**
  - `GET /api/admin/processing` returns the exact flattened Zod snapshot of §7.3: generated time/revision/apply state/topology, queue rows with controls+runtime+blockers, pipeline controls+counters+blockers, service controls+gate+cached health, vectors and storage; all collections follow topology order and every nullable “not measured” value is explicit.
  - Independent monitor/repository reads run in parallel. `POST /api/admin/processing/services/check` uses the documented cache semantics, never runs through a queue or gate, strips URL userinfo and publishes no credential; an absent first sample (`UNKNOWN`/`null`) is distinct from `DOWN`.
  - PATCH routes for queue/pipeline/step/service accept the scoped commands of M61.2 and return `{ revision, changed, apply, controls, resumed }`. Unknown topology path names return `422 VALIDATION_FAILED`; stale revision and safe-apply failures use their documented status/code.
  - `GET /api/admin/processing/failures`, `POST /api/admin/processing/failures/:jobId/retry` and `POST /api/admin/processing/reprocess` retain the established pagination and command contracts under the unified namespace. Every new mutating route is refused to a read-only bearer token by both HTTP enforcement layers and every route requires `ADMIN` through `RolesGuard`.
  - The old `/api/admin/queue/*` endpoints remain documented compatibility endpoints for this transition: the whole settings writer delegates to serialized safe `replaceLegacy`, failures/reprocess delegate to the new processing commands, and analysis retains its separate established contract. New UI code is forbidden from calling the legacy whole settings/overview/services/failures/reprocess routes.
  - Shared Zod contracts validate the snapshot and every command/response. Control-plane tests prove topology order, liveness passthrough, cached-health `UNKNOWN` and that ordinary GET never probes; health tests cover sampling/cache behavior; the exhaustive route-table test proves admin and document-access guards; web tests parse and use only the shared contracts. Compatibility-path coverage is indexed under M61.8.

- [x] **M61.4 — A queued request is never acknowledged away**
  **Goal:** overlapping document work and manual retry preserve every requested run and every central queue policy.
  **Depends on:** M61.1 (may be implemented alongside M61.2–M61.3, but merges after M61.3 under the sequential milestone rule).
  **Docs:** [`05 §5.4f`](../05-library-and-processing.md#delivery-and-retry-stay-behind-the-queue-port), [`05 §5.5`](../05-library-and-processing.md#55-document-processing-pipeline-document-process), [`06 §6.8`](../06-backend-architecture.md#68-queue-integration-pg-boss)
  **Acceptance:**
  - Replace the busy-document early return with a per-document promise chain: only one run mutates a document at once, and every delivery which reaches the handler runs whole after its predecessor and does not resolve until its own work settles.
  - Handler-level coalescing is forbidden: pg-boss `short` already performs the safe debounce while matching jobs wait, while an identical full payload arriving during an active run may represent a composition changed after that run read it. Disjoint subsets and `analyseInFull` semantics are likewise not lost.
  - Failure of the active or following run settles each pg-boss job according to its own result; the coordinator releases its in-memory entry after success or failure and cannot leave a settled promise or document lock behind.
  - `QueueMonitor` becomes read-only. Manual Retry loads the original failed name/payload, rejects an unknown queue and calls `JobQueue.enqueue`; retry limit, backoff, expiry, priority/defaults and derived singleton key are attached only by the adapter. The original failed row remains history.
  - Deterministic handler tests block the first run and prove a second identical full request and a disjoint subset wait rather than overlap or disappear, prove a failed predecessor does not discard its successor, then prove another delivery starts after either outcome releases the chain. Retry tests prove a manual copy goes through `JobQueue`; adapter tests prove retry/backoff/expiry/singleton policy is attached there, with no direct queue `boss.send` outside that adapter.

- [x] **M61.5 — The screen reads as one processing system**
  **Goal:** `/admin/processing` is a single understandable management surface driven only by the processing snapshot and scoped commands.
  **Depends on:** M61.3 and M61.4.
  **Docs:** [`11 §11.13`](../11-ui-ux-spec.md#1113-admin-processing-adminprocessing), [`10 §10.2–§10.5`](../10-frontend-architecture.md)
  **Acceptance:**
  - The navigation and heading say **Processing**. Overview/Pipeline/Services/Failures keep addressable tabs under `/admin/processing/:tab`; old `/admin/queue` tab URLs redirect to their exact equivalents and unknown new tabs answer 404 without a nested `loading.tsx` boundary.
  - Overview renders the server topology strip and queue operational evidence. Pipeline renders detailed dependency/resource relationships and Services renders the reverse consumer view from the same snapshot objects/edges, with no `serviceOfStep`, copied node list or client-derived dependency blocker table.
  - Every numeric row has an isolated draft and sends only its scoped command with the revision it was edited from. A success clears that draft and revalidates the snapshot; a revision conflict keeps the draft and refreshes the evidence. The shared apply-state banner renders `APPLIED`, `APPLIED_WITH_WARNINGS` or `DEGRADED` and the server detail without inventing a client-side success state.
  - Effective values visibly distinguish default from override and **Use default** sends `null` for one field. A pause shows its direct/inherited `blockers` beside the affected step; shared gate waiting is labelled service-wide rather than assigned to a guessed step.
  - Service probing uses `POST …/services/check` at the slower documented cadence without blanking the rest of the screen. Failures stay paginated and Retry preserves the row as history; snapshot/load-control/probe/failure/reprocess calls use `/api/admin/processing/*`, while analysis language keeps its documented queue compatibility path because it is not a load control.
  - English reference messages and Russian translations cover every new label, role/condition, blocker, provenance/apply state and error. Component tests cover the snapshot/apply banner, topology and liveness rendering, isolated queue draft, stale-revision draft preservation, scoped queue/pipeline/step/service commands, reset-to-default, explicit/periodic health checks, cursor continuation, failure retry and new-tab navigation. Route tests cover canonical tabs, exact legacy redirects, unknown tabs and the loading-boundary invariant; the four data tables use horizontal max-content overflow.

- [x] **M61.6 — A document names the pause which actually holds it**
  **Goal:** the viewer distinguishes a directly paused step from a downstream step conditionally held by that pause.
  **Depends on:** M61.3 and M61.5.
  **Docs:** [`05 §5.4d, §5.4f`](../05-library-and-processing.md), [`07 §7.3`](../07-api-specification.md#the-pipeline-as-everybody-sees-it), [`11 §11.5`](../11-ui-ux-spec.md#115-document-viewer-documentsidtab)
  **Acceptance:** a signed-in document-scoped processing-state response evaluates topology blockers against that document's existing canonical/Markdown artifacts and type, without publishing admin-only depths or settings; it distinguishes direct pause, parent queue pause and dependency pause, including `UPSTREAM_UNSETTLED`, `UPSTREAM_UNSETTLED_AND_TYPE_MISSING` and the conditional analysis → fields edge; the viewer names the upstream cause under every affected `PENDING` step and does not falsely call a downstream step held where an earlier artifact/type lets it proceed; authorization is identical to reading the document. Server and viewer tests cover canonical → preview/Markdown, Markdown → analysis/fields/vectorization, typed vs untyped fields, existing-artifact bypass, parent queue pause and no pauses; the exhaustive route-table test covers document access, and the viewer test proves the legacy global paused-step endpoint is never called.

- [x] **M61.7 — Depth becomes evidence that work is moving**
  **Goal:** an operator can tell an idle queue, a healthy backlog and a stale backlog apart without reading pg-boss tables.
  **Depends on:** M61.3 and M61.5.
  **Docs:** [`05 §5.4f, §5.8`](../05-library-and-processing.md), [`07 §7.3`](../07-api-specification.md#admin-processing), [`11 §11.13`](../11-ui-ux-spec.md#1113-admin-processing-adminprocessing)
  **Acceptance:** one grouped, bounded `QueueMonitor` query returns for every topology queue `queued`, `active`, `failedRecent`, `oldestQueuedAt`, `lastCompletedAt` and `completedLastHour`, with `null` rather than epoch/fabricated dates when pg-boss retains no evidence; the snapshot passes those fields through without inventing per-step completions; Overview localizes the timestamps/ages, hides meaningless zero failures and presents enough server evidence for an operator to distinguish no retained history from queued or recently completed work. Monitor and integration tests cover absent rows, created/retry work, a completion inside and outside the rolling hour, last completion and stable known-queue order; the component test covers liveness passthrough alongside resolved settings and topology.

- [x] **M61.8 — The old surface is only an alias, and the specification agrees**
  **Goal:** the migration ends with one implementation path, complete coverage and no contradictory queue-era documentation.
  **Depends on:** M61.1–M61.7.
  **Docs:** [ADR-026](../02-architecture-overview.md#adr-026-one-processing-control-plane-three-execution-mechanisms), [`05 §5.4f`](../05-library-and-processing.md#54f-one-processing-control-plane), [`06 §6.3.5, §6.8`](../06-backend-architecture.md), [`07 §7.3`](../07-api-specification.md#admin-processing), [`11 §11.13`](../11-ui-ux-spec.md#1113-admin-processing-adminprocessing), [`12 §12.4`](../12-build-config-run.md#124-envexample)
  **Acceptance:** the legacy whole settings writer is a thin compatibility delegate and old overview/health/failures/reprocess paths use the same underlying readers/control-plane commands rather than a second implementation; only the new processing routes appear in processing client code, while the separate analysis-language compatibility route is listed once and allowed. Queue bootstrap, live commands, maintenance, document reprocess and viewer blockers share the closed topology identities and `QueueSettings` source without pretending they are one runtime abstraction; English/Russian copy consistently names all six steps. Focused server/web integration suites, `npm run typecheck`, `npm run lint`, `npm test` and `npm run build` pass; the scenario-coverage map below stays synchronized with the actual test files. An architecture search proves no queue `boss.send` outside `PgBossJobQueue`, no presentation-layer worker/gate sequencing and no client-side queue/step/service relationship map.

### M61 scenario-coverage map

This is an audit index, not a substitute for the acceptance criteria in the task boxes above. A row counts only when
the named assertions and the full milestone checks pass; when a scenario moves, this table changes in
the same commit.

| Scenario | Authoritative test files and proof |
|---|---|
| Safe apply and compensation | `src/server/application/processing/processing-control-plane.test.ts` — desired persistence failure touches no runtime; runtime failure restores settings/workers under a new revision; failed settings or worker compensation publishes `DEGRADED` with `appliedRevision: null`; a later failed persistence attempt cannot hide that degraded runtime. `src/server/application/queue/queue-settings.test.ts` separately proves monotonic restore and provenance restoration. |
| Stale revision and isolated drafts | `src/server/application/processing/processing-control-plane.test.ts` — concurrent commands are serialized and the second stale revision is refused under the mutation lock. `src/web/screens/admin-queue/admin-queue-screen.test.tsx` — the scoped request carries its base revision and a conflict preserves that row's draft while revalidating evidence. |
| Overlapping document delivery | `src/server/application/jobs/handle-document-process.test.ts` — a blocked first run keeps both an identical full delivery and a changed subset behind it; a failed predecessor still runs its successor; a later delivery proves the chain was released after settlement. `test/integration/queue.integration.test.ts` — pg-boss `short` deduplicates matching queued work but keeps different requested-step keys distinct. |
...
- [x] **M62.7 — Receipt failures can be counted and retried from Processing**
