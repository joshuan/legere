...
users convenient access: viewing, search (full-text and semantic), categories, sharing.
...
3. The queue processes the files (volumes can be large — thousands of files at once): first-page
   preview, text extraction to Markdown, categorization, vectorization.
...
| **User** | `USER` | Views documents available to them; searches; organizes (folders/collections, categories); shares; triggers scan-set merging |
...
| **Job** | A unit of work in the processing queue: library scan, hashing, parsing, preview, categorization, vectorization, scan-set merge |
| **Category** | A document type from a managed reference list (passport, contract, invoice, manual…); assigned automatically, editable manually |
...
- Pipeline: canonicalization to PDF → first-page JPG preview → Markdown extraction (with OCR for
  scans) → categorization → vectorization.
...
- Scan sets: manual file selection → merge into a PDF with margin cropping (Stirling-PDF) → a derived
  document.
- Admin panel: libraries, users/invites, category reference list, queue and error monitoring.
