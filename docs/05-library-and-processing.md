...
|---|---|---|
| Pixels one image is **worked on** at | 80 Mpx — reached by downscaling, not by refusing; 268 Mpx opened at all | sharp's own limit is a ceiling on what libvips can address (~268 Mpx), not a budget. A 16383×16383 single-colour PNG is a few hundred KB — far under `UPLOAD_MAX_BYTES` — and decodes to ~805 MB of raw RGB, with the perspective warp of §5.6 allocating an output of the same order. 80 Mpx clears the worst legitimate case (an A3 sheet at 600 dpi is 69.7 Mpx) and is what every raster below is measured against. A picture **above** it is not refused: refusing one is how the simplest step in the pipeline became the one that could not render a large scan — five documents on a live instance FAILED at the canonical step and two at the preview, all of them a photograph carrying the library's own "Input image exceeds pixel limit". A 100-megapixel phone photograph is a page like any other. Its size is read off its header, which decodes no pixels, and the decode is then opened with a `resize` that brings the raster under the bound before anything else touches it; libvips streams that shrink a scanline at a time, so what is held is the raster being written rather than the one being read — measured, a 268 Mpx PNG costs 87 MB of resident memory on the way to a preview and 557 MB on the way to a full-bound raster, against 46 MB and 506 MB for a picture already at the bound. The step **records that it did so**, because a page that arrived at less than the resolution it was photographed at is a fact about the archive. Past 268 Mpx there is no shrink to stream a picture through, and the step fails naming the size it was handed — `30000×30000 (900.0 Mpx)` — rather than repeating a library's words at somebody who never chose the limit |
...
1. **Canonical PDF — for every document, always** (ADR-021, ADR-025). One artifact, `canonical.pdf`,
   ...
   2. **Each page becomes a PDF part**, `unitConcurrency` of them at a time. 🔒 **A picture larger
      than the pixel budget is made smaller, not refused** (§5.4a): every place this step opens a
      picture — the crop, the turn, the correction, the shape it measures, and the preview of step 2
      after it — reads the size off the header first and, past the budget, brings the raster under it
      on the way in, keeping the aspect ratio the page will be laid out by and recording in the log
      that the page arrived at less than the resolution it was photographed at. A 100-megapixel scan
      is a page like any other; what the budget still refuses is a picture no shrink can reach, and
      it says which picture and how large. Then, per page:
